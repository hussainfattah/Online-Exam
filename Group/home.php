...
if (window.XMLHttpRequest) {
	...
	xmlhttp.onreadystatechange=function()
	{
		...
		{
			document.getElementById("parent_t").innerHTML=xmlhttp.responseText;
...
function update(){
var status=document.getElementById("status_s").value;
if (window.XMLHttpRequest) {
		// code for IE7+, Firefox, Chrome, Opera, Safari
		xmlhttp=new XMLHttpRequest();
	} else { // code for IE6, IE5
	...
	xmlhttp.onreadystatechange=function() {
		if (xmlhttp.readyState==4 && xmlhttp.status==200) {
			...
			document.getElementById("status_s").value="";
		}
	}
	xmlhttp.open("GET","status_process.php?status="+status,true);
	xmlhttp.send();
}
</script>
</head>
<body>
	...
	<br/>
	<br/>
	<textarea id="status_s" name="status" cols="126" rows="5" style="margin-left:10px;margin-bottom:60px;color:white;
